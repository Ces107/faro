...
def legal_notice_html(business: BusinessProfile) -> str:
    ...
    address = f"{business.address}, {business.city}" if business.address else business.city
    email = f"<li>Email: {business.email}</li>" if business.email else ""
    return f"""<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Aviso legal · {business.name}</title>
<style>
...
<body>
  ...
  {business.name} y facilitar el contacto con el negocio.</p>
  ...
  <p>Los contenidos de este sitio pertenecen a {business.name} o se usan con
